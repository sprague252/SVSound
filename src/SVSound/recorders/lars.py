...
by a Loggerhead Instruments LARS device.
"""
from __future__ import division

def get_info(file, info={}):
    """info = get_info(file, info={}) Read the information in a WAV file
    written by a Loggerhead Instruments LARS device and return the
    contents. The standard information in the fmt chunk is included in
    the info dictionary. In addtition, the date string decoded from the
    filename is returned in info["datestring".]
    ...
    info dictionary keys and values returned
        ...
        "datestring" - an string containing the ISO date string for
            the start of the recording obtained from the file name.
    ...
    fname = basename(file.name).split('.')
    # Filename should start with 'LH'.
    if fname[0][:2] != 'LH':
        raise ValueError('File name does not begin with LH: ', fname[0])
    # Get the datestring for the recording frome the filename.
    info['datestring'] = dt.datetime.fromtimestamp(int(fname[0][2:])).isoformat()
    # Get the usual WAVE file info.
    file.seek(0)
    data = file.read(4)
    if data != b'RIFF':
